...
//Global variables
 ...
 var SING_COLOR_BIAS_FACTOR=2.0; //If the player is committed to only 1 color, reduce bias by this factor
  ...
  for (i = 0; i < pack_length; i++) {
    ...
    denom=COLOR_COMMIT_THRESHOLD / MAX_BONUS_SPEC; 
     ...
     if(num_colors==2){
       ...
       //Check the number of colors the player is committed to
       num_player_colors=0;
       for (var kk=0;kk<5;kk++){
       ...
       if (num_card_colors==1){
         ...
         //Case 2b: if committed to only one color, reduce bias by factor SING_COLOR_BIAS_FACTOR
         if (num_player_colors==1){
